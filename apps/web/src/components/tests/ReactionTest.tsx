...
type GameState = 'instructions' | 'waiting' | 'ready' | 'clicked' | 'tooEarly' | 'results';
...
export function ReactionTest({ onComplete }: ReactionTestProps) {
  ...
  const handleClick = useCallback(async () => {
    ...
    if (gameState === 'ready') {
      ...
      if (newRounds.length >= totalRounds) {
        ...
        const average = Math.round(newRounds.reduce((a, b) => a + b, 0) / newRounds.length);
        try {
          ...
          if (isMountedRef.current && testResult) {
            ...
            setResult({ id: 'local', testType: 'reaction', score: average, percentile: 50, createdAt: new Date(), userId: 'local' });
            setGameState('results');
          }
        } catch (e) {
  ...
  }, [gameState, startTime, rounds, currentRound, onComplete, startRound, clearGameTimeout, isProcessing]);
  ...
  const getBackgroundColor = () => {
    switch (gameState) {
      case 'waiting': return 'bg-destructive';
      case 'ready': return 'bg-success';
      case 'tooEarly': return 'bg-warning';
    ...
    >
      {gameState === 'instructions' && (
        <div className="text-center animate-slide-up">
          ...
          <p className="text-muted-foreground mb-8 max-w-sm">
            When the red box turns <span className="text-success font-semibold">green</span>, 
            click as quickly as you can! {totalRounds} rounds total.
          </p>
      ...
      {gameState === 'waiting' && (
        <div className="text-center">
          ...
          <p className="text-destructive-foreground/80">Round {currentRound + 1} of {totalRounds}</p>
        </div>
      )}

      {gameState === 'ready' && (
        <div className="text-center">
          <h2 className="text-4xl font-bold text-success-foreground animate-pulse">Click now!</h2>
        </div>
      )}
