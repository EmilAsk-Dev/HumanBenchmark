...
export function TestCard({ testType, stats, index = 0 }: TestCardProps) {
  ...
  const getTrendIcon = () => {
    if (!stats || stats.history.length < 2) return <Minus className="h-4 w-4 text-muted-foreground" />;
        ...
        <div className="space-y-2">
          ...
          <div className="mt-3">
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className={cn(
                'font-medium',
                stats.percentile >= 90 ? 'text-yellow-500' : 
                stats.percentile >= 70 ? 'text-primary' : 
