...
type GameState = 'instructions' | 'showing' | 'hidden' | 'levelComplete' | 'results';

export function ChimpTest({ onComplete }: ChimpTestProps) {
  const navigate = useNavigate();
  const [gameState, setGameState] = useState<GameState>('instructions');
  const [level, setLevel] = useState(4);
  const [numbers, setNumbers] = useState<{ value: number; x: number; y: number; clicked: boolean }[]>([]);
  ...
  const handleCellClick = useCallback(async (clickedNum: { value: number; x: number; y: number }) => {
    ...
    if (gameState === 'showing') {
      if (clickedNum.value === 1) {
        sounds.click();
        setGameState('hidden');
        setNumbers(prev => prev.map(n => n.value === 1 ? { ...n, clicked: true } : n));
        setNextExpected(2);
      }
      return;
    }
    ...
    if (gameState === 'hidden') {
      if (clickedNum.value === nextExpected) {
        ...
        sounds.error();
        const finalLevel = Math.max(4, level);
        ...
        try {
          ...
          if (isMountedRef.current && testResult) {
            ...
            setResult({ id: 'local', testType: 'chimp', score: finalLevel, percentile: 50, createdAt: new Date(), userId: 'local' });
            setGameState('results');
          }
        } catch (e) {
  ...
  }, [gameState, nextExpected, level, numbers, onComplete, startLevel, isProcessing]);
  ...
  // Reset level to 4 when entering instructions state
  useEffect(() => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 animate-scale-in bg-background">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-foreground mb-2">Chimp Test Complete</h2>
          ...
          <p className="text-muted-foreground mt-2">
            {result.score >= 9 ? "Amazing! You beat the average chimp!" : 
             result.score >= 7 ? "Great memory!" : "Keep practicing!"}
          </p>
        </div>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-6 animate-slide-up bg-background">
        ...
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-8 max-w-sm">
            Numbers will appear on screen. Memorize their positions, then click them in order starting from 1.
            Can you beat a chimpanzee?
          </p>
          <Button className="gradient-primary text-primary-foreground" onClick={handleStart}>
            Start Test
          </Button>
        </div>
      </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-background">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Get ready for level {level + 1}...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-background">
      ...
      <div className="text-center mb-4">
        ...
        {gameState === 'showing' && (
          <p className="text-sm text-muted-foreground">Memorize positions, then click 1</p>
        )}
        {gameState === 'hidden' && (
          <p className="text-sm text-primary font-medium">Click {nextExpected}</p>
        )}
      </div>
